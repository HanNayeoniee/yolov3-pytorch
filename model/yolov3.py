...
class YOLODetection(nn.Module):
    def __init__(self, anchors, image_size: int, num_classes: int):
        ...
        self.num_anchors = len(anchors)  # 3개
        self.num_classes = num_classes  # 80개
        self.image_size = image_size  # 416
        self.mse_loss = nn.MSELoss()  # mean squared error
        self.bce_loss = nn.BCELoss()  # 클래스가 2개인 binary case인 경우에 사용
    ...
    def forward(self, x, targets):
        ...
        num_batches = x.size(0)  # 배치사이즈, 입력값이 [1, 3, G, G]이므로 1
        grid_size = x.size(2)  # 입력값이 [1, 3, G, G]이므로 G, G값은 13/ 26/ 52

        # 출력값 형태 변환
        prediction = (
            x.view(num_batches, self.num_anchors, self.num_classes + 5, grid_size, grid_size)
                .permute(0, 1, 3, 4, 2).contiguous()
        )  # prediction의 shape변화 : (1, 3, 85, G, G) -> (1, 3, G, G, 85)

        # Get outputs
        # cx, cy, w, h, pred_conf의 shape은 모두 같음
        cx = torch.sigmoid(prediction[..., 0])  # 앵커박스의 Center x, shape은 [1, 3, G, G]
        cy = torch.sigmoid(prediction[..., 1])  # Center y
        w = prediction[..., 2]  # Width
        h = prediction[..., 3]  # Height
        pred_conf = torch.sigmoid(prediction[..., 4])  # Object confidence (objectness)
        pred_cls = torch.sigmoid(prediction[..., 5:])  # Class prediction, shape은 [1, 3, G, G, 80]

        # Calculate offsets for each grid
        stride = self.image_size / grid_size  # 416/g = 32/ 16/ 8
        ...
        scaled_anchors = torch.as_tensor([(a_w / stride, a_h / stride) for a_w, a_h in self.anchors],
                                         dtype=torch.float, device=device)  # 앵커박스 크기/ stride
        ...
        # 우리가 예측한 박스와 ground truth박스간의 iou등
        # target은 ground truth
        ...
        # cx, cy, w, h는 예측값, tx, ty, tw, th는 정답(ground truth)
        ...
        loss_conf_obj = self.bce_loss(pred_conf[obj_mask], tconf[obj_mask])  # 물체가 있을 때 있다고 예측했는지
        loss_conf_no_obj = self.bce_loss(pred_conf[no_obj_mask], tconf[no_obj_mask])  # 물체가 없을 때 없다고 예측했는지
        # self.obj_scale=1, self.no_obj_scale=100
        # 물체가 없는데 물체가 있다고 예측한 경우에는 패널티를 더 많이 줌
        ...
        precision = torch.sum(iou50 * detected_mask) / (conf50.sum() + 1e-16)  # 정밀도
        recall50 = torch.sum(iou50 * detected_mask) / (obj_mask.sum() + 1e-16)  # 재현율
        recall75 = torch.sum(iou75 * detected_mask) / (obj_mask.sum() + 1e-16)  # 재현율
...
class YOLOv3(nn.Module):
    ...
    def forward(self, x, targets=None):
        ...
        yolo_outputs = torch.cat(yolo_outputs, 1).detach().cpu()  # 3개의 feature map에서 나온 output을 합침
        return yolo_outputs if targets is None else (loss, yolo_outputs)
        # return scale1, scale2, scale3  # loss, output을 구하지 않고 feature map 3개만 뽑는 경우
...
#     # model = YOLOv3(Darknet(), num_classes=80)
#     # test = torch.rand([1, 3, 416, 416])
#     # y = model(test)
#     #
#     # print(f'Yolo feature1 shape : {y[0].shape}')
#     # print(f'Yolo feature2 shape : {y[1].shape}')
#     # print(f'Yolo feature3 shape : {y[2].shape}')
#
#     # 원본 코드
